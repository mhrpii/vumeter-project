...
# Boylece her Linux'ta calisir (Scarlett'e bagimli degil).
...
MAC_AUDIO_SOURCE = None   # Linux: kaynak _find_scarlett_monitor() ile bulunur
...
# Mac: sysmon_mac (iStats+psutil), Linux: sysmon (lm-sensors). Once sistemi dene.
try:
    import sys as _sysm
    import sysmon as _sysmon_mod   # Linux: lm-sensors + hwmon
except Exception:
    try:
    ...
    def draw_disk_row(items, row_top, row_h, tag):
        ...
        for idx, (model, temp, typ) in enumerate(items):
            ...
            if True:
                ...
                bx = cx0 + 7
                bt = row_top + int(row_h * 0.06)
                bh = int(row_h * 0.56)   # yuzde yazisi disk adiyla cakismasin
...
# ==================== KAYNAK BULUCU ====================
def _find_cava_bin():
    """Linux'ta cava'yi bul (Homebrew: /opt/homebrew Apple Silicon, /usr/local Intel)."""
    import shutil
...
def write_cava_config(bars=NUM_BARS, fps=60, autosens=1):
    """Linux: PipeWire monitor kaynagi (pulse), profil bagimsiz bulunur."""
    _src = _find_scarlett_monitor()
...
def _find_scarlett_monitor():
    """Linux: Scarlett'in AKTIF PipeWire monitor kaynagini profil bagimsiz bul.
    pro-audio -> pro-output-0.monitor, HiFi -> Line1__sink.monitor.
    Once RUNNING olani, yoksa herhangi Scarlett .monitor, o da yoksa
    varsayilan cikisin monitoru."""
    try:
...
class CavaReader:
    ...
    def pause_for_sleep(self):
        ...
        Scarlett serbest kalir -> coreaudiod uyku engelini birakir -> bilgisayar uyur."""
    ...
    def _restart_cava(self):
        ...
        # ile isit. bilgisayar uykudan kalkinca cava/portaudio loopback'e baglanamiyor
    ...
    def _wake_audio(self):
        """Linux: PipeWire uyku sonrasi kendini toparliyor, isitma gerekmez."""
        return
                ...
                if len(parts) >= NUM_BARS:
                    ...
                    if max(vals) <= 1:
                        ...
                        if self._zero_since is None:
                            ...
                            # Linux'ta PipeWire profil degisimi icindi. Linux'ta
                            ...
                            # DUZELTME: sessizlik ARIZA DEGIL (muzik durmus/
            ...
            try:
                ...
                if deneme == 15:
                    try:
                        ...
                        _sp.Popen(["notify-send", "VU Meter LCD",
                                   "Panel baglanamiyor - USB kablosunu cikarip yeniden takin"])
                    except Exception:
                        pass
    ...
    def _position_near_tray(w):
        ...
        try:
            ...
            if geo.width() > 0:
                ...
                # tray ekranin alt yarisindaysa (Linux gorev cubugu) pencere
                # YUKARI acilmali; ust yarisindaysa (macOS menu cubugu) asagi.
                if geo.top() > scr.center().y():
                    y = geo.top() - w.height() - 6
                else:
                    y = geo.bottom() + 6
            else:
                # tray konumu yoksa (Wayland'da geometry bos doner):
                # Linux -> sag ALT, macOS -> sag UST
                import sys as _sp2
                x = scr.right() - w.width() - 12
                if _sp2.platform == "darwin":
                    y = scr.top() + 30
                else:
                    # tray ikonuna yaklastir: pencere genisligi kadar sola kaydir
                    x = scr.right() - (w.width() * 2) - 12
                    y = scr.bottom() - w.height() - 12
            # ekran disina tasmasin
            x = max(scr.left() + 6, min(x, scr.right() - w.width() - 6))
            y = max(scr.top() + 6, min(y, scr.bottom() - w.height() - 6))
            w.move(int(x), int(y))
        except Exception:
            pass
...
def main():
    ...
    def render_loop():
        ...
        while _state["running"]:
            ...
            # UYKU-DOSTU CAVA YONETIMI (bilgisayar uyuyabilsin):
            # Kullanici 120s+ hareketsiz VE muzik yoksa -> cava durur (Scarlett
            # serbest, coreaudiod uyku engelini birakir -> bilgisayar uyur).
